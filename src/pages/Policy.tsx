...
const policies = {
  ...
  terms: { title: "Terms of Service", body: "By using libani, you agree to these terms. Prohibited activities include scraping, abuse, fraudulent orders, and reselling for resale outside Pakistan. We reserve the right to cancel suspicious orders. Our liability is limited to the purchase price of the item." },
  shipping: { title: "Shipping Policy", body: "We deliver across Pakistan. Delivery charges apply on every order — we charge only when we deliver. Damaged or wrong items are replaced free of charge — just notify us within 48 hours of delivery." },
} as const;
...
export default function Policy() {
  ...
  const body = slug === "shipping"
    ? `We deliver across Pakistan within ${settings.delivery_days_min}-${settings.delivery_days_max} working days. Delivery charges of ${formatPKR(Number(settings.shipping_fee || 0))} apply on every order — we charge only when we deliver. Damaged or wrong items are replaced free of charge — just notify us within 48 hours of delivery.`
    : p.body;
  return (
    <>
      <SEO title={`${p.title} | libani`} description={body.slice(0, 150)} />
      <div className="container-x py-6">
        ...
        <div className="mt-6 grid md:grid-cols-[220px_1fr] gap-8">
          ...
          <article>
            ...
            <p className="mt-4 text-sm text-muted-foreground leading-relaxed whitespace-pre-line">{body}</p>
          </article>
        </div>
      </div>
    </>
